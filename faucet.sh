...
post_sendbtc() {
   ...
   # format: address\n
   read ADDR
   if [ -z "$(echo "$ADDR" | egrep "^[123456789ABCDEFGHJKLMNPQRSTUVWXYZabcdefghijkmnopqrstuvwxyz]{27,33}$")" ]; then 
      http_401
      return 3
   fi
      ...
      GET)
         case "$REQPATH" in
            ...
            /bitcoin/confirmations/*)
               local TXID="${REQPATH#/confirmations/}"
            ...
            /bitcoin/utxos/*)
               local ADDR="${REQPATH#/utxos/}"
               if ! [[ "$ADDR" =~ ^[123456789ABCDEFGHJKLMNPQRSTUVWXYZabcdefghijkmnopqrstuvwxyz]{27,33}$ ]]; then 
                  http_401
                  STATUS=401
               else
                  get_utxos "$ADDR"
...
# look up staging DB
STACKS_WORKING_DIR="$(fgrep 'working_dir' "$STACKS_CONF" | cut -d ' ' -f 3 | sed -r 's/"//g')"
if ! [ -d "$STACKS_WORKING_DIR" ]; then
   exit_error "No such file or directory: $STACKS_WORKING_DIR"
fi
